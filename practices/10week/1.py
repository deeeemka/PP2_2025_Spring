...
def insert_from_csv(file_path):
    ...
    with open(file_path, 'r') as f:
        ...
        for row in reader:
            cur.execute(
                "INSERT INTO phone_book (first_name, last_name, phone) VALUES (%s, %s, %s)",
                row
            )
    conn.commit()
    cur.close()
...
def update_phonebook(first_name, new_phone):
    cur = conn.cursor()
    cur.execute(
        "UPDATE phone_book SET phone = %s WHERE first_name = %s",
...
def query_phonebook(first_name):
    cur = conn.cursor()
    cur.execute("SELECT * FROM phone_book WHERE first_name = %s", (first_name,))
    rows = cur.fetchall()
    for row in rows:
        print(row)
    cur.close()
...
def delete_from_phonebook(by, value):
    cur = conn.cursor()
    cur.execute(f"DELETE FROM phone_book WHERE {by} = %s", (value,))
    conn.commit()
    cur.close()
    # DO NOT close conn here
...
def main():
    while True:
        ...
        if choice == '1':
            ...
            insert_from_csv(file_path)  # Pass the variable, not a literal string
        elif choice == '2':
            insert_via_console()
        elif choice == '3':
    ...
    conn.close()  # Close the connection once when exiting
